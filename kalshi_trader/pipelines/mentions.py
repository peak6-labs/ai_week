...
from kalshi_trader.external.mentions_parser import parse_mention_title, extract_phrase_from_settlement, base_rate_from_points
...
def main() -> None:
    ...
    async def run() -> None:
        ...
        try:
            ...
            if parsed is None:
                # Title doesn't contain the keyword — try settlement context.
                settlement = parse_settlement_arg(args.settlement_json)
                parsed = extract_phrase_from_settlement(settlement)
                if parsed is None:
                    print("NO_KEYWORD: keyword not found in title or settlement context", file=sys.stderr)
                    print(json.dumps([]))
                    return
            ...
            if base_rate["period_count"] == 0:
                print(f"NO_COVERAGE: phrase '{parsed['phrase']}' has no historical GDELT TV coverage on {parsed['station']}", file=sys.stderr)
                print(json.dumps([]))
                return
            ...
            if parsed.get("is_sports_venue"):
                estimate.metadata["station_note"] = (
                    f"Sports venue: settled on game broadcast, but queried {parsed['station']} "
                    "as a TV-news proxy (GDELT does not index sports broadcasts directly)."
                )
