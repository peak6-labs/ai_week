"""Trade executor — submits approved risk-checked ideas as live Kalshi orders."""
from __future__ import annotations

import math
from datetime import datetime

from kalshi_trader.models import OrderAction, OrderResult, RiskDecision, Side, TradeIdea
...
class TradeExecutor:
    ...
    async def execute(self, idea: TradeIdea, decision: RiskDecision) -> OrderResult:
        ...
        if count < 1:
            raise ValueError(
                f"Position too small: ${decision.approved_size_dollars:.2f} "
                f"at {price_dollars:.2f}/contract"
            )
